...
connect(async (err) => {
  if (!err) {
    ...
    if (collections.length === 0) {
      ...
      customerCol.createIndex({ phoneNumber: 1 }, { unique: true })
      customerCol.createIndex({ email: 1 }, { unique: true })
...
async function genInvoicePdf (data, res) {
  ...
  doc.moveTo(cord.price, 160)
    ...
    .stroke()
  doc.text('  #            SKU                    Item                               Qty         Price           Total', -120, 170)
  data.items.forEach((item, index) => {
    const indent = 5
    const lineSpace = 15
    doc.text(`${index + 1}`, cord.hash - indent - 10, 200 + (lineSpace * index))
      .text(`${item.sku}`, cord.hash + indent, 200 + (lineSpace * index))
      .text(`${item.name}`, cord.sku + indent, 200 + (lineSpace * index))
      .text(`${item.amount}`, cord.item + indent, 200 + (lineSpace * index))
      .text(`R${item.price}`, cord.qty + indent, 200 + (lineSpace * index))
      .text(`R${item.amount * item.price}`, cord.price + indent, 200 + (lineSpace * index))
    if (index + 1 === data.items.length) {
      doc.text('********************', cord.sku + indent + 20, 200 + (lineSpace * index + 20))
    }
  })
